...
def position_camera(
    ...
    phi = math.radians(elevation_float)
    ...
    # rx, ry, rz = math.pi/2, 0, math.pi/2
    # # latitude
    # rx = rx - phi # elevation
    # # longtitude
    # rz = rz + theta # azimuth
...
def render_object(
    ...
    for i in range(num_renders):
        ...
        # save camera position and Distance, Azimuth and Elevation
        if random_camera:
            x = camera.location[0] 
            y = camera.location[1] 
            z = camera.location[2]
            ...
            elevation = math.degrees(math.asin(z/distance))
            azimuth = (math.degrees(math.atan2(x/distance, y/distance)) + 360.) % 360.
        ...
        aed_list_path = os.path.join(output_dir, f"{i:03d}_aed.json")
        with open(aed_list_path, "w") as outfile: 
            json.dump(aed_dict, outfile)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--random_camera",
        action="store_true",
        help="rendering from random viewpoints",
        default=False,
    )
    parser.add_argument(
        "--random_lights",
    ...
    render.image_settings.file_format = g_rgb_file_format
    render.image_settings.color_mode = g_rgb_color_mode
    render.resolution_x = g_resolution_x
    render.resolution_y = g_resolution_y
    render.resolution_percentage = g_resolution_percentage

    # Set cycles settings
    scene.cycles.device = g_cycles_device
    scene.cycles.samples = g_cycles_samples
    scene.cycles.diffuse_bounces = 1
    scene.cycles.glossy_bounces = 1
    scene.cycles.transparent_max_bounces = 3
    scene.cycles.transmission_bounces = 3
    scene.cycles.filter_width = 1 # 0.01
    ...
    render_object(
        ...
        random_camera=args.random_camera,
